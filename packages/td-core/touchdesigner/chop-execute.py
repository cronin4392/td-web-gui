...
def _webgui():
	...
	if comp is None:
		raise RuntimeError("chop-execute: no global OP shortcut 'WebGuiServer' - "
...
def _config():
	...
	if dat is None:
		raise RuntimeError("chop-execute: WebGuiServer has no 'config' DAT - "
...
def _callbacks():
	...
	if dat is None:
		...
		raise RuntimeError("chop-execute: WebGuiServer has no DAT '%s' - "
