...
Optional, and orthogonal to `REGISTRY`/`READOUTS`: `HANDLERS` lets the web
**invoke behaviour**, not just read/write values — "run this now, here are
some arguments, tell me what came back." See
[protocol.md § Calls](protocol.md#calls) for the wire contract.
...
JSON-serializable replies `result_not_serializable`. `HANDLERS` degrades
gracefully like `WEBRTC`/`STREAMS`: a config predating this section is still a
valid params-only project.

**TD → web**: call `parent.WebGuiServer.Notify(name, args)` for fire-and-forget
(broadcasts to every connected client), or
`parent.WebGuiServer.Call(name, args, on_result=fn, on_error=fn, timeout=10.0)`
for a reply via callback — from the Textport, an Execute DAT, anywhere in
project code. `Call()` **never blocks**: TD's main thread runs the frame, and
waiting synchronously for a browser reply would freeze it. It targets
the sole connected client by default; with zero or more than one connected, it
invokes `on_error('call_disconnected', ...)` immediately rather than guessing
which browser should get it — the same single-viewer stance the video wiring
above already takes. The web registers what it answers to with
`createTDHandler('name', fn)` inside a component, or `connection.handle()`
outside one. `createTDHandler` registers on the nearest `<Provider>` of any
factory; with several instances on one page, use
`Mixer.useConnection().handle('name', fn)` to pin it to a named one. See
[api.md § Scoping](api.md#scoping).

Nothing here is generated by `Rebuild()` — a handler is just a Python function,
so there's no watcher DAT to keep in sync.
...
Schemas are per-instance, so each gets its own `createTDClient<Schema>()`. Each
factory's controls bind the nearest provider **from that factory**, so the
providers can sit side by side or nest, and a control for one instance can render
inside another's subtree. See
[api.md § Multiple instances](api.md#multiple-instances) and
[api.md § Scoping](api.md#scoping).
...
| [`config-template.py`](../touchdesigner/config-template.py)         | **Yes** | Your registry, readouts, handlers, wiring names, and stream map  |
