...
def _webgui():
	...
	if comp is None:
		raise RuntimeError("webrtc-callbacks: no global OP shortcut 'WebGuiServer' - "
...
def _config():
	...
	if dat is None:
		raise RuntimeError("webrtc-callbacks: WebGuiServer has no 'config' DAT - "
...
def _announce_streams(connectionId, sdp):
	...
	if len(mids) < len(streams):
		...
		print("webrtc-callbacks: warning - %d stream(s) configured but the SDP "
			  "carries %d video m-line(s); raise `receivers` on the web-side "
			  "<Provider video={{ receivers: N }}> to carry them all"
			  % (len(streams), len(mids)))

	_relay(connectionId, {'type': 'streams', 'streams': [
