...
rather than inferred:
        intensity  → level_master's Brightness inside the wall
        enabled    → /project1/moviefilein_source's Play
        reset      → /project1/parexec_restart, which cues that movie

The rest stay parameters only. That is the normal case — a control surface's job
is to set parameters — and /project1/par_params → trail_params is where you
watch any of them arrive.
...
Video additionally expects:
        ...
        /project1/videowall           the four-tile wall: in_source → res_cap →
                                      level_master → level_tile1…4 → video1…4.
                                      It ends at those Nulls; the encoders are
...
#   enabled: optional, default True — the state the stream starts a session in,
