...
describe('round-trip', () => {
  ...
  it('a rewrite fully replaces prior contents — no orphaned rows', () => {
    ...
    const orphanPhrases = db.prepare('SELECT COUNT(*) AS n FROM phrases WHERE tab_id = ?').get('old') as {
      n: number
    }
...
describe('integrity', () => {
  it('ON DELETE CASCADE removes a dropped tab\'s phrases', () => {
    ...
    const remaining = db.prepare('SELECT COUNT(*) AS n FROM phrases WHERE tab_id = ?').get('doomed') as {
      n: number
    }
