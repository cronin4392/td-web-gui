...
export function readLibrary(db: DatabaseSync): Library {
  ...
  const recent = (db.prepare('SELECT phrase FROM recent ORDER BY position').all() as { phrase: string }[]).map(
    (r) => r.phrase,
  )
...
export function writeLibrary(db: DatabaseSync, library: Library): void {
  ...
  try {
    ...
    const insertPhrase = db.prepare('INSERT INTO phrases (tab_id, phrase, position) VALUES (?, ?, ?)')
